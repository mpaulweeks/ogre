...
export class Board {
  ...
  private get2dGrid(allSquares: OgreSquare[]) {
    ...
    const xMin = Math.min(-1, ...allPoints.map(p => p.x)) - 1;
    const xMax = Math.max(1, ...allPoints.map(p => p.x)) + 1;
    const yMin = Math.min(-2, ...allPoints.map(p => p.y)) - 1;
    const yMax = Math.max(2, ...allPoints.map(p => p.y)) + 1;
